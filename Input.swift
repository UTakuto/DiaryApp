            ...
            ZStack {
                ...
                if showWhiteScreen {
                    Color.white
                        .ignoresSafeArea()
                        .transition(.opacity)
                        .animation(.easeInOut(duration: 1), value: showWhiteScreen) // 1秒かけてフェードイン
                }

                // フェードで次の画面に遷移
                if showNextScreen {
                    NavigationLink(
                        destination:CompleteView(), // 新しいファイルに定義された画面に遷移
                        isActive: $showNextScreen,
                        label: {
                            EmptyView() // ここでは空のビューを使用
                        }
                    )
                    .transition(.opacity) // フェードアニメーションを設定
                }
            }
        }
    }
